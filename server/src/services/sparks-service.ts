...
export async function getLiveProgress(
  ...
  // Карточка «твои искры за вчера»: самый свежий отданный, но ещё не открытый
  // ребёнком день. Сундук приходит всем, кто был на смене, а не только тем, кому
  // досталось что-то сверх: день присутствия начисляется каждому, и «пустых»
  // дней у ребёнка из ростера не бывает.
  const pending = days.filter((d) => !d.opened).pop() ?? null;
